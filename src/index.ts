...
export type Page = puppeteer.Page & {
    ...
    waitForAnyOfSelectors: (selectors: string[], timeout?: number) => Promise<null | string>;
    clickUntilSelectorAppears: (selector: string, waitingForThisSelector: string, timeout?: number) => Promise<boolean>;
    clickUntilSelectorDisappears: (selector: string, timeout?: number) => Promise<boolean>;
    waitForChildNodeAmountToChange: (selector: string, timeout?: number) => Promise<number>;
...
const replaceBrowserFunctions = async (browser: Browser): Promise<Browser> => {
    ...
    browser.launchPage = async function (viewportWidth = 1920, viewportHeight = 1080): Promise<Page> {
    ...
    browser.resetBrowser = async (viewportWidth = 1920, viewportHeight = 1080) => {
        const page = await browser.launchPage(viewportWidth, viewportHeight);
...
const replacePageFunctions = async (page: Page): Promise<Page> => {
    ...
    page.click = async (selector: string, timeout = 30000) => {
        if (typeof timeout !== 'number') throw new Error('click: the type of "timeout" must be a number');
    ...
    page.waitForAnyOfSelectors = async (selectors, timeout = 30000): Promise<null | string> => {
    ...
    page.clickUntilSelectorAppears = async (selector, waitingForThisSelector, timeout = 30000): Promise<boolean> => {
        ...
        throw new Error('page.clickUntilSelectorAppears: we never got selector: ' + waitingForThisSelector);
    };

    page.clickUntilSelectorDisappears = async (selector, timeout = 30000): Promise<boolean> => {
        const start = Date.now();
        await page.waitForSelector(selector);
        while (Date.now() - start < timeout) {
    ...
    page.waitForChildNodeAmountToChange = async (selector: string, timeout = 30000): Promise<number> => {
        const start = Date.now();
        await page.waitForSelector(selector);
