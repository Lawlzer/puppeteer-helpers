...
const exists = async (page, selector, timeout = 100) => {
    if (typeof page !== 'object') {
        console.log('In custom function Helpers.exists(page, selector, timeout), page is not an object.');
...
const closeBrowser = async (browser) => { // stops an inconsistent error
...
const click = async (page, selector, timeout = 30000) => {
    if (typeof page !== 'object') {
        console.log('Helpers.click was not passed in page as the first argument.');
        throw new Error();
    }
    await page.waitForSelector(selector, { timeout: timeout });
...
const resetBrowser = async (browser) => {
    if (browser) await closeBrowser(browser); // first time this runs, we won't have a browser
    return await launchBrowser();
}
exports.resetBrowser = resetBrowser;

const waitForAnySelector = async (page, selectors, timeout = 30000, log = true) => {
    if (typeof page !== 'object') {
        console.log('in waitForAnySelector, page was not passed in as an object. Throwing.');
        throw new Error();
    }
    ...
    while (true) {
        ...
        if (Date.now() - startTime > timeout) {
            if (log) console.log('None of the possibilities showed up in waitForAnySelector. Returning null. Input selectors: ', selectors);
            return null;
        }
...
const type = async (page, selector, text) => {
    if (typeof page !== 'object') {
        console.log('in Helpers.type, order should be page(type, selector, text). page was not an object.');
        throw new Error();
    }
    if (typeof selector !== 'string') {
        console.log('in Helpers.type, order should be page(type, selector, text). text was not a string.');
        throw new Error();
    }
    if (typeof text !== 'string') {
        console.log('in Helpers.type, order should be page(type, selector, text). text was not a strings.');
        throw new Error();
    }
...
const waitForNumberInnerHTML = async (page, selector, timeout = 30000) => {
    ...
    console.log('Error in waitForNumber: waited for over 30 seconds, did not get a number. Throwing error. Selector: ', selector);
    throw new Error();
}
...
const waitForChildNodeToChange = async (page, selector, timeout = 30000) => {
    ...
    console.log('Error in waitForChildNodeToChange: waited for over 30 seconds, did not change. Throwing error. Selector: ', selector);
...
const clickUntilSelectorAppears = async (page, selector, waitForThis, timeout = 30000) => {
    ...
    console.log('in clickUntilSelectorAppears, we never got selector: ', waitForThis); 
...
const clickUntilSelectorDisappears = async (page, selector, timeout = 30000) => {
    ...
    console.log('Selector never appeared in clickUntilSelectorDisappears: ' + selector); 
